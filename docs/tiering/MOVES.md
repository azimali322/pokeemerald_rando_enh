# Randomizable Moves — tiering worksheet

Pool: `sRandomValidMoves[]` (src/pokemon.c) — **367 moves**.
Every move `GetRandomMove()` can return. Mark each with a tier number in the `[ ]`.

Suggested scale (mirror the ability list): `1`=S `2`=A `3`=B `4`=C `5`=D `6`=F/junk. `X` = exclude entirely.

TM moves are marked `[TM]`, HM moves `[HM]`. **HM moves must never be randomized** (soft-lock risk).
...
```
  ...
  [ ] MOVE_CUT                 [HM]    pow= 50  acc= 95  GRASS    PHYS pp=30
  ...
  [ ] MOVE_FLY                 [HM]    pow= 80  acc= 95  FLYING   PHYS pp=15
  ...
  [ ] MOVE_SURF                [HM]    pow= 90  acc=100  WATER    SPEC pp=15
  ...
  [ ] MOVE_STRENGTH            [HM]    pow= 80  acc=100  NORMAL   PHYS pp=15
  ...
  [ ] MOVE_WATERFALL           [HM]    pow= 80  acc=100  WATER    PHYS pp=15
  ...
  [ ] MOVE_FLASH               [HM]    pow=  0  acc=100  NORMAL   STAT pp=20
  ...
  [ ] MOVE_ROCK_SMASH          [HM]    pow= 40  acc=100  FIGHTING PHYS pp=15
  ...
  [ ] MOVE_DIVE                [HM]    pow= 90  acc=100  WATER    PHYS pp=10
